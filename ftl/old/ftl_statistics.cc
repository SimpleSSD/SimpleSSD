...
void FTLStats::print_epoch_stats(Tick sim_time) {
  ...
  if (sim_time % EPOCH_INTERVAL != 0)
    epoch_number++;

  collect_epoch_stats(epoch_number);

  // DPRINTF(FTLOut, "FTL Host epoch %d , time: %" PRIu64 " \n", epoch_number,
  // sim_time); DPRINTF(FTLOut, "FTL Host read count %d \n", (int)
  // host_epoch_read_count.Get() ); DPRINTF(FTLOut, "FTL Host read size %lf
  // KB\n", host_epoch_read_size.Get()); DPRINTF(FTLOut, "FTL Host read latency
  // %lf us\n", host_epoch_read_latency.Get()); DPRINTF(FTLOut, "FTL Host read
  // capacity %lf KB \n", host_epoch_read_capacity.Get()); DPRINTF(FTLOut, "FTL
  // Host write count %d \n", (int)host_epoch_write_count.Get());
  // DPRINTF(FTLOut, "FTL Host write size %lf KB\n",
  // host_epoch_write_size.Get()); DPRINTF(FTLOut, "FTL Host write latency %lf
  // us\n", host_epoch_write_latency.Get()); DPRINTF(FTLOut, "FTL Host write
  // capacity %lf KB\n", host_epoch_write_capacity.Get());

  // BW
  // DPRINTF(FTLOut, "FTL Host read  BW (active): %lf MB/s \n",
  // host_epoch_read_BW_active.Get()); DPRINTF(FTLOut, "FTL Host write BW
  // (active): %lf MB/s \n", host_epoch_write_BW_active.Get()); DPRINTF(FTLOut,
  // "FTL Host rw    BW (active): %lf MB/s \n", host_epoch_rw_BW_active.Get());
  // DPRINTF(FTLOut, "FTL Host read  BW (total ): %lf MB/s \n",
  // host_epoch_read_BW_total.Get()); DPRINTF(FTLOut, "FTL Host write BW (total
  // ): %lf MB/s \n", host_epoch_write_BW_total.Get()); DPRINTF(FTLOut, "FTL
  // Host rw    BW (total ): %lf MB/s \n", host_epoch_rw_BW_total.Get());
  // DPRINTF(FTLOut, "FTL Host read  BW (only  ): %lf MB/s \n",
  // host_epoch_read_BW_only.Get()); DPRINTF(FTLOut, "FTL Host write BW (only
  // ): %lf MB/s \n", host_epoch_write_BW_only.Get());

  // IOPS
  // DPRINTF(FTLOut, "FTL Host read  IOPS (active): %lf \n",
  // host_epoch_read_IOPS_active.Get()); DPRINTF(FTLOut, "FTL Host write IOPS
  // (active): %lf \n", host_epoch_write_IOPS_active.Get()); DPRINTF(FTLOut,
  // "FTL Host rw    IOPS (active): %lf \n", host_epoch_rw_IOPS_active.Get());
  // DPRINTF(FTLOut, "FTL Host read  IOPS (total ): %lf \n",
  // host_epoch_read_IOPS_total.Get()); DPRINTF(FTLOut, "FTL Host write IOPS
  // (total ): %lf \n", host_epoch_write_IOPS_total.Get()); DPRINTF(FTLOut, "FTL
  // Host rw    IOPS (total ): %lf \n", host_epoch_rw_IOPS_total.Get());
  // DPRINTF(FTLOut, "FTL Host read  IOPS (only  ): %lf \n",
  // host_epoch_read_IOPS_only.Get()); DPRINTF(FTLOut, "FTL Host write IOPS
...
void FTLStats::collect_epoch_stats(int ep_num) {
  ...
  if (current_epoch_read_count != 0) {
    current_epoch_read_latency =
        current_epoch_read_lat_sum / (double)current_epoch_read_count;
    current_epoch_read_size =
        current_epoch_read_size_sum / (double)current_epoch_read_count;
  }
  if (current_epoch_write_count != 0) {
    current_epoch_write_latency =
        current_epoch_write_lat_sum / (double)current_epoch_write_count;
    current_epoch_write_size =
        current_epoch_write_size_sum / (double)current_epoch_write_count;
  }
  ...
  host_epoch_read_BW_total.update((current_epoch_read_capacity * USEC * USEC) /
                                  (KBYTE * total_time));
  host_epoch_write_BW_total.update(current_epoch_write_capacity * USEC * USEC /
                                   (KBYTE * total_time));
  host_epoch_rw_BW_total.update(
      (current_epoch_read_capacity + current_epoch_write_capacity) * USEC *
      USEC / (KBYTE * total_time));
  host_epoch_read_IOPS_total.update(current_epoch_read_count * USEC * USEC /
                                    (total_time));
  host_epoch_write_IOPS_total.update(current_epoch_write_count * USEC * USEC /
                                     (total_time));
  host_epoch_rw_IOPS_total.update(
      (current_epoch_read_count + current_epoch_write_count) * USEC * USEC /
      (total_time));
  ...
  if ((current_epoch_read_count == 0) || (current_epoch_read_capacity == 0)) {
    ...
    host_epoch_read_BW_active.update(current_epoch_read_capacity * USEC * USEC /
                                     (KBYTE * active_time));
    host_epoch_read_BW_only.update(current_epoch_read_capacity * USEC * USEC /
                                   (KBYTE * read_active_time));
    host_epoch_read_IOPS_active.update(current_epoch_read_count * USEC * USEC /
                                       (active_time));
    host_epoch_read_IOPS_only.update(current_epoch_read_count * USEC * USEC /
                                     (read_active_time));
  }
  ...
  if ((current_epoch_write_count == 0) || (current_epoch_write_capacity == 0)) {
    ...
    host_epoch_write_BW_active.update(current_epoch_write_capacity * USEC *
                                      USEC / (KBYTE * active_time));
    host_epoch_write_BW_only.update(current_epoch_write_capacity * USEC * USEC /
                                    (KBYTE * write_active_time));
    host_epoch_write_IOPS_active.update(current_epoch_write_count * USEC *
                                        USEC / (active_time));
    host_epoch_write_IOPS_only.update(current_epoch_write_count * USEC * USEC /
                                      (write_active_time));
  }

  if ((current_epoch_read_count + current_epoch_write_count == 0) ||
      (current_epoch_read_capacity + current_epoch_write_capacity == 0)) {
    host_epoch_rw_BW_active.update(0);
    host_epoch_rw_IOPS_active.update(0);
  }
  else {
    host_epoch_rw_BW_active.update(
        (current_epoch_read_capacity + current_epoch_write_capacity) * USEC *
        USEC / (KBYTE * active_time));
    host_epoch_rw_IOPS_active.update(
        (current_epoch_read_count + current_epoch_write_count) * USEC * USEC /
        (active_time));
  }
}
...
// bool myfunction(pair<Tick, int> i, pair<Tick, int> j){return (i.first <
// j.first);}
Tick FTLStats::epoch_active_time(vector<RequestInterval> *ev,
                                 int epoch_number) {
  Tick at = 0;
  ...
  std::sort(sort_list.begin(), sort_list.end(),
            [](pair<Tick, int> const &a, pair<Tick, int> const &b) {
...
void FTLStats::update_stats_for_request(Command *cmd, int ep_num) {
  ...
  if (ep_num == last_epoch_collected + 1) {
    ...
    if (cmd->operation == OPER_READ) {
      ...
      current_epoch_read_size_sum +=
          cmd->size / 1024;  // convert from sector to KB
      current_epoch_read_lat_sum +=
          cmd->getLatency() / USEC;  // convert from ps to us
      ...
      current_epoch_write_size_sum +=
          cmd->size / 1024;  // convert from sector to KB
      current_epoch_write_lat_sum +=
          cmd->getLatency() / USEC;  // convert from ps to us
    ...
    if (cmd->operation == OPER_READ) {
      ...
      next_epoch_read_size_sum +=
          cmd->size / 1024;  // convert from sector to KB
      next_epoch_read_lat_sum +=
          cmd->getLatency() / USEC;  // convert from ps to us
      ...
      next_epoch_write_size_sum +=
          cmd->size / 1024;  // convert from sector to KB
      next_epoch_write_lat_sum +=
          cmd->getLatency() / USEC;  // convert from ps to us
...
void FTLStats::read_req_arrive(Tick arrive_time) {
  ...
  if (arrive_time < read_active_last_update)
    return;
...
void FTLStats::write_req_arrive(Tick arrive_time) {
  ...
  if (arrive_time < write_active_last_update)
    return;
...
void FTLStats::rw_req_arrive(Tick arrive_time) {
  ...
  if (arrive_time < rw_active_last_update)
    return;
...
void FTLStats::read_req_leave(Tick leave_time) {
  ...
  if (leave_time < read_active_last_update)
    return;
...
void FTLStats::write_req_leave(Tick leave_time) {
  ...
  rw_req_leave(leave_time);
  sim_write_outstanding_count--;
  if (leave_time < write_active_last_update)
    return;
...
void FTLStats::rw_req_leave(Tick leave_time) {
  sim_rw_outstanding_count--;
  if (leave_time < rw_active_last_update)
    return;
...
Tick FTLStats::get_write_active_time(Tick current_time) {
  // make sure active time is updated up to now
  // if (current_time != write_active_last_update && write_active_last_update !=
  // 0)
...
void FTLStats::print_simulation_stats(Tick sim_time) {
  ...
  //    // DPRINTF(FTLOut, "FTL Host sim read  size ");
  //    host_sim_read_size.print();
  //    // DPRINTF(FTLOut, "FTL Host sim read  latency ");
  //    host_sim_read_latency.print();
  ...
  //    // DPRINTF(FTLOut, "FTL Host sim write size ");
  //    host_sim_write_size.print();
  //    // DPRINTF(FTLOut, "FTL Host sim write latency ");
  //    host_sim_write_latency.print();
  ...
  if (active_time != 0) {
    host_sim_read_BW_active.update(
        host_sim_read_capacity * USEC * USEC /
        (KBYTE * active_time));  // to convert ps to s
    host_sim_write_BW_active.update(host_sim_write_capacity * USEC * USEC /
                                    (KBYTE * active_time));
    host_sim_rw_BW_active.update(
        (host_sim_read_capacity + host_sim_write_capacity) * USEC * USEC /
        (KBYTE * active_time));

    host_sim_read_IOPS_active.update(host_sim_read_count * USEC * USEC /
                                     (active_time));
    host_sim_write_IOPS_active.update(host_sim_write_count * USEC * USEC /
                                      (active_time));
    host_sim_rw_IOPS_active.update(
        (host_sim_read_count + host_sim_write_count) * USEC * USEC /
        (active_time));
  }
  if (total_time != 0) {
    host_sim_read_BW_total.update(host_sim_read_capacity * USEC * USEC /
                                  (KBYTE * total_time));
    host_sim_write_BW_total.update(host_sim_write_capacity * USEC * USEC /
                                   (KBYTE * total_time));
    host_sim_rw_BW_total.update(
        (host_sim_read_capacity + host_sim_write_capacity) * USEC * USEC /
        (KBYTE * total_time));

    host_sim_read_IOPS_total.update(host_sim_read_count * USEC * USEC /
                                    (total_time));
    host_sim_write_IOPS_total.update(host_sim_write_count * USEC * USEC /
                                     (total_time));
    host_sim_rw_IOPS_total.update((host_sim_read_count + host_sim_write_count) *
                                  USEC * USEC / (total_time));
  }
  if (read_active_time != 0) {
    host_sim_read_BW_only.update(host_sim_read_capacity * USEC * USEC /
                                 (KBYTE * read_active_time));
    host_sim_read_IOPS_only.update(host_sim_read_count * USEC * USEC /
                                   (read_active_time));
  }
  if (write_active_time != 0) {
    host_sim_write_BW_only.update(host_sim_write_capacity * USEC * USEC /
                                  (KBYTE * write_active_time));
    host_sim_write_IOPS_only.update(host_sim_write_count * USEC * USEC /
                                    (write_active_time));
  }
